...
export function registerAssetTools(server: McpServer, fileIO: FileIO): void {
  ...
  server.tool(
    "import_image",
    "Import an image file (png/jpg/jpeg/webp/svg) into gamekit/assets and register it on the project.",
    {
      id: z.string().describe("Desired Asset ID (kebab-case, e.g., 'hero')"),
      sourcePath: z.string().describe("Absolute path to the source image"),
    },
    async ({ id, sourcePath }) => {
      ...
      const asset = { id, file: fileName, kind: "image" as const };
      project.assets.push(asset);
      await fileIO.writeProject(project);
      await regenerateAssetsManifest(fileIO.projectRoot, project);
      return {
