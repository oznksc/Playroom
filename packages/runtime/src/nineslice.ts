...
export function computeNineSliceRegions(
  ...
  if (midSrcW > 0 && top > 0) regions.push({ sx: left, sy: 0, sw: midSrcW, sh: top, x: xL, y: y0, w: midW, h: top });
  if (midSrcW > 0 && bottom > 0) regions.push({ sx: left, sy: srcHeight - bottom, sw: midSrcW, sh: bottom, x: xL, y: yB, w: midW, h: bottom });
  if (midSrcH > 0 && left > 0) regions.push({ sx: 0, sy: top, sw: left, sh: midSrcH, x: x0, y: yT, w: left, h: midH });
  if (midSrcH > 0 && right > 0) regions.push({ sx: srcWidth - right, sy: top, sw: right, sh: midSrcH, x: xR, y: yT, w: right, h: midH });
  if (left > 0 && top > 0) regions.push({ sx: 0, sy: 0, sw: left, sh: top, x: x0, y: y0, w: left, h: top });
  if (right > 0 && top > 0) regions.push({ sx: srcWidth - right, sy: 0, sw: right, sh: top, x: xR, y: y0, w: right, h: top });
  if (left > 0 && bottom > 0) regions.push({ sx: 0, sy: srcHeight - bottom, sw: left, sh: bottom, x: x0, y: yB, w: left, h: bottom });
  if (right > 0 && bottom > 0) regions.push({ sx: srcWidth - right, sy: srcHeight - bottom, sw: right, sh: bottom, x: xR, y: yB, w: right, h: bottom });
  return regions;
}