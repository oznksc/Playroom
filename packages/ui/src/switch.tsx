...
>(({ className, size = "sm", variant = "accent", ...props }, ref) => {
  ...
  const activeColor = {
    accent:
      "data-[state=checked]:bg-accent data-[state=checked]:shadow-[0_0_8px_rgba(0,240,255,0.35)]",
    success:
      "data-[state=checked]:bg-accent-green data-[state=checked]:shadow-[0_0_8px_rgba(16,185,129,0.35)]",
  }[variant];
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col">
          {label && (
            <span className="text-base font-medium text-text-primary leading-tight">{label}</span>
          )}
          {description && (
            <span className="text-2xs text-text-muted leading-tight mt-0.5">{description}</span>
          )}
        </div>
      </label>
    );
