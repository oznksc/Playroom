...
>(({ className, size = "sm", children, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex w-full items-center justify-between gap-2 border border-border-default bg-surface-sunken font-medium tracking-[-0.01em] text-text-primary outline-none select-none",
...
>(({ className, children, position = "popper", sideOffset = 4, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border-default bg-[rgba(18,20,24,0.95)] p-1 text-text-primary shadow-glass backdrop-blur-[20px]",
        position === "popper" &&
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn(
      "px-2 py-1 text-2xs font-semibold tracking-[-0.01em] text-text-muted uppercase",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-7 text-xs tracking-[-0.01em] text-text-primary outline-none",
  ...
  ({ className, selectSize = "md", children, ...props }, ref) => {
    ...
    return (
      <select
        ref={ref}
        className={cn(
          "flex w-full appearance-none border border-border-default bg-surface-sunken bg-[length:12px] bg-[right_8px_center] bg-no-repeat pr-7 tracking-[-0.01em] text-text-primary outline-none",
          "transition-[border-color,box-shadow,background] duration-150",
          ...
          "hover:border-border-strong hover:bg-black/35",
          "focus:border-accent/50 focus:shadow-[0_0_0_2px_rgba(0,240,255,0.15)]",
          "disabled:cursor-not-allowed disabled:opacity-40",
          sizeClasses,
          className
        )}
...
export const Select = React.forwardRef<any, SelectProps>((props, ref) => {
  if (
    ...
    ("value" in props && !("onChange" in props) && !("className" in props))
  ) {
