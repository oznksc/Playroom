...
import { Box, Plus, Focus, Trash2 } from "lucide-react";
import { useState } from "react";
import { findComponent } from "../lib/components.js";
import { Select, Button, EmptyState, IconButton, Input } from "@/ui";
...
export function Inspector({
  ...
  const collider = entity
    ? findComponent<AabbColliderComponent>(entity, "AabbCollider")
    : undefined;
  const circleCollider = entity
    ? findComponent<CircleColliderComponent>(entity, "CircleCollider")
    : undefined;
  const polygonCollider = entity
    ? findComponent<PolygonColliderComponent>(entity, "PolygonCollider")
    : undefined;
  const player = entity
  ...
  const audioSource = entity
    ? findComponent<AudioSourceComponent>(entity, "AudioSource")
    : undefined;
  const audioListener = entity
    ? findComponent<AudioListenerComponent>(entity, "AudioListener")
    : undefined;
  ...
  const stateMachine = entity
    ? findComponent<StateMachineComponent>(entity, "StateMachine")
    : undefined;
  const script = entity ? findComponent<ScriptComponent>(entity, "Script") : undefined;
  const particleSystem = entity
    ? findComponent<ParticleSystemComponent>(entity, "ParticleSystem")
    : undefined;
  ...
  if (entity) {
    ...
    if (MVP_SHOW_ADVANCED_PHYSICS && !circleCollider)
      missingComponents.push({ val: "CircleCollider", label: "Circle Collider 2D" });
    if (MVP_SHOW_ADVANCED_PHYSICS && !polygonCollider)
      missingComponents.push({ val: "PolygonCollider", label: "Polygon Collider 2D" });
    if (!player) missingComponents.push({ val: "PlayerController", label: "Player Controller" });
    if (MVP_SHOW_ADVANCED_PHYSICS && !rigidBody)
      missingComponents.push({ val: "RigidBody", label: "RigidBody 2D" });
    ...
    if (!particleSystem)
      missingComponents.push({ val: "ParticleSystem", label: "Particle System" });
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {missingComponents.map((comp) => (
                    <option key={comp.val} value={comp.val}>
                      {comp.label}
                    </option>
                  ))}
                </Select>
