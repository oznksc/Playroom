...
import { Skull, Plus, Trash2, Flag, Trophy } from "lucide-react";
import {
  NumberField,
  Select,
  AccordionSection,
...
type SceneOnChange = (mutator: (scene: GameKitScene) => void) => void;
...
type GameRulesSectionProps = {
  scene: GameKitScene;
  open: boolean;
  onToggle: () => void;
  onChange: SceneOnChange;
};

export function GameRulesSection({ scene, open, onToggle, onChange }: GameRulesSectionProps) {
  const rules = resolveGameRules(scene.gameRules);

  return (
    <AccordionSection icon={<Skull size={12} />} label="Game rules" open={open} onToggle={onToggle}>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <NumberField
            ...
            onChange={(value) =>
              onChange((draft) => {
                const gr = ensureGameRules(draft);
                gr.spawnPoint = { x: value, y: gr.spawnPoint?.y ?? 0 };
              })
            }
          />
          <NumberField
            ...
            onChange={(value) =>
              onChange((draft) => {
                const gr = ensureGameRules(draft);
                gr.spawnPoint = { x: gr.spawnPoint?.x ?? 0, y: value };
              })
            }
          />
        </div>
      ...
      <div className="border-t border-border-default pt-2 mt-2 space-y-1.5">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-[10px] font-semibold tracking-[-0.01em] text-text-muted">Mode</span>
          <Select
      ...
      <div className="border-t border-border-default pt-2 mt-2 space-y-1.5">
        ...
        <p className="text-[10px] text-text-muted">
          Empty = default <span className="font-mono">completeLevel</span> when a scene manager is
          present.
        </p>
