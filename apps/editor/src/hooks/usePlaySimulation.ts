...
export function usePlaySimulation({
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      const input = mergeGamepadIntoInput(baseInput, sceneRef.current?.inputMap, pollGamepad());
      ...
      while (accumulator >= fixedDt && steps < maxSteps) {
        ...
        const nextEntities: GameKitEntity[] = workingScene.entities.map((entity) => {
          ...
          const transform = ent.components.find(
          ...
          if (rb) {
            ...
            const aabbCollider = ent.components.find(
              (c): c is AabbColliderComponent => c.type === "AabbCollider"
            );
            const circleCollider = ent.components.find(
              (c): c is CircleColliderComponent => c.type === "CircleCollider"
            );
            const polygonCollider = ent.components.find(
              (c): c is PolygonColliderComponent => c.type === "PolygonCollider"
            );
            ...
            const rbComp = ent.components.find(
            ...
            const collider = ent.components.find(
              (c): c is AabbColliderComponent => c.type === "AabbCollider"
            );
            const circleCollider = ent.components.find(
              (c): c is CircleColliderComponent => c.type === "CircleCollider"
          ...
          const followPath = ent.components.find(
          ...
          const sm = ent.components.find(
            (c): c is StateMachineComponent => c.type === "StateMachine"
          );
          if (script || sm) {
            const context = rulesEngineRef.current?.scriptContext(ent.id, {
        ...
        for (const event of triggerUpdate.events) {
          addConsoleLog(
            "physics",
            `Trigger ${event.type}: ${event.triggerEntityId} with ${event.otherEntityId}`
          );
          if (event.type === "enter") {
            ...
            for (const entityId of [event.triggerEntityId, event.otherEntityId]) {
              ...
              const script = entity.components.find(
                (c): c is ScriptComponent => c.type === "Script"
              );
              if (script) {
        ...
        for (const event of collisionUpdate.events) {
          addConsoleLog(
            "physics",
            `Collision contact: ${event.entityId} with ${event.otherEntityId}`
          );
        }
  ...
  const applyPlaySceneRuntime = useCallback(
    (nextScene: GameKitScene, manager: SceneManager | null) => {
  ...
  const handlePlayToggle = useCallback(async () => {
    if (!isPlaying) {
      ...
      playUnlockedLevelIdsRef.current = manager
        .getLevels()
        .filter((l) => l.unlocked)
        .map((l) => l.id);

      playHotSwapRef.current = (sceneId: string) => {
        const ok = manager.switchScene(sceneId) || manager.switchScene(normalizeSceneFile(sceneId));
        if (!ok) return false;
        const resolved =
      ...
      if (USE_PHASER_PLAY_HOST) {
        ...
        addConsoleLog(
          "system",
          "Camera follows player inside the game screen only (canvas pan locked)."
        );
        addConsoleLog("system", "Menu shell: GUI buttons can switchScene during play.");
        if ((audioControllerRef.current?.sources.length ?? 0) > 0) {
          addConsoleLog(
            "system",
            `Audio: ${audioControllerRef.current!.sources.length} source(s) armed.`
          );
        }
      }
    } else {
  ...
  const onVirtualInput = useCallback(
    (action: "left" | "right" | "jump" | "fire" | "action", pressed: boolean) => {
        ...
        evaluateScriptEvent(action, script, {
          ...
          sceneManager: {
            ...
            getPersistentVar: (key, defaultValue) =>
              manager?.getPersistentVar(key, defaultValue) ??
  ...
  const onPhaserOutcome = useCallback(
    (kind: "won" | "lost", message: string) => {
      if (kind === "won") {
        playOutcomeRef.current = "win";
        setPlayOutcome({ kind: "win", message, livesLeft: playLivesRef.current });
        setIsPaused(true);
