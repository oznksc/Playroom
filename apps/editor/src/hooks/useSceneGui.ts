...
export function useSceneGui({
  ...
  const [selectedComponentInstanceId, setSelectedComponentInstanceId] = useState<string | null>(
    null
  );
  const [editingComponentId, setEditingComponentId] = useState<string | null>(null);

  const addGuiNode = useCallback(
    (type: GuiNode["type"]) => {
      updateScene((draft) => {
        const base = {
          id: createId(type),
  ...
  const deleteGuiNode = useCallback(
    (id: string) => {
      updateScene((draft) => {
  ...
  const updateGuiNode = useCallback(
    (mutator: (node: GuiNode) => void) => {
  ...
  const addGuiComponent = useCallback(
    (name: string) => {
  ...
  const deleteGuiComponent = useCallback(
    (componentId: string) => {
  ...
  const addNodeToEditingComponent = useCallback(
    (type: GuiNode["type"]) => {
      ...
      switch (type) {
        case "Text":
          node = { ...base, type: "Text", text: "Text", fontSize: 16, color: "#ffffff" };
          break;
        case "Button":
          node = {
            ...base,
  ...
  const deleteNodeFromEditingComponent = useCallback(
    (nodeId: string) => {
      if (!editingComponentId) return;
      const newComponents = snapshot.guiComponents.map((c) =>
  ...
  const addGuiComponentInstance = useCallback(
    (componentId: string) => {
      updateScene((draft) => {
  ...
  const deleteGuiComponentInstance = useCallback(
    (instanceId: string) => {
      updateScene((draft) => {
        draft.gui.componentInstances = (draft.gui.componentInstances ?? []).filter(
          (i) => i.id !== instanceId
  ...
  const updateGuiComponentInstance = useCallback(
    (mutator: (inst: GuiComponentInstance) => void) => {
      if (!selectedComponentInstanceId) return;
      updateScene((draft) => {
        const inst = (draft.gui.componentInstances ?? []).find(
