...
export function useSceneEntities({
  ...
  const addTemplateEntity = useCallback((templateType: "empty" | "sprite" | "collider" | "player" | "camera") => {
    updateScene((draft) => {
  ...
  const handleSpawnEntityWithSprite = useCallback((assetId: string, width: number, height: number, category?: string) => {
    updateScene((draft) => {
  ...
  const handleAttachAudioToEntity = useCallback((assetId: string, isBgm?: boolean) => {
    updateScene((draft) => {
      if (isBgm) {
  ...
  const pasteEntity = useCallback((source: GameKitEntity) => {
    updateScene((draft) => {
  ...
  const selectEntity = useCallback((id: string, shift = false) => {
    clearGuiSelection();
    if (!id) {
