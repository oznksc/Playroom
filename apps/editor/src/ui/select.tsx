...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, children, ...props }, ref) => (
    <select
      ref={ref}
      className={cn(
        "flex h-[26px] w-full appearance-none rounded-md border border-border-default bg-bg-base bg-[length:12px] bg-[right_6px_center] bg-no-repeat px-2 pr-6 text-base text-text-primary outline-none transition-[border-color,box-shadow] duration-[120ms]",
