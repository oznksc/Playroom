...
| Text | ✅ | ✅ | `text`, `fontAssetId`, `size`, `color`, `align`, optional `width` (word wrap) on both. Missing `fontAssetId`/unloaded font falls back to a system font on both (Skia `matchFont`, Phaser family stack). |
...
| RigidBody | ✅ | ✅ | Both honor `velocity`, `mass`, `angularVelocity`, `isKinematic`, `gravityScale`, `drag`, `useGravity`. `applyImpulse` script actions work on both (impulse ÷ mass; no-op on kinematic). Drag uses the shared exponential per-frame model and bodies fall asleep after `RIGID_BODY_SLEEP_DELAY` of stillness while supported, waking on input/impulse — identical on both. |
...
| `anchorX`/`anchorY` / `nodeOverrides` (GUI) | ✅ | ✅ | Both honor `anchorX`/`anchorY` (0..1 fractions; `x`/`y` stays the top-left when unset) and `nodeOverrides` per instance. Shared `offsetGuiNode`/`guiNodeOrigin` in `@gamekit/runtime/gui` drive Skia, Phaser, and the editor canvas preview. |
